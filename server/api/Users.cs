    ...
    async Task<IResult> CreateAdmin(RegisterRequest registerRequest)
    {
        ...
        try
        {
            await using (var reader = await cmd.ExecuteReaderAsync())
            {
                ...
                {
                    await using var cmd2 = Db.CreateCommand("INSERT INTO users (username, password, role, email, company) VALUES (@username, @password, 'ADMIN', @email, @company_id) RETURNING id;");
